...
## Post-Phase-10 addition — live smoke test

**Date:** 2026-07-29

Directly motivated by the incident above: no automated check had ever run against a genuinely *deployed* instance before — every test suite (unit, integration) exercises business logic against a real database with a *mocked* Stripe client, which is a different layer from "is this actual running process even up and wired correctly." `scripts/smoke-test.ts` (new) is a plain HTTP client - not an in-process script like the others in `scripts/` - that hits any running instance (local, CI, or deployed) and proves every one of the 18 registered routes is reachable, the Phase 10 admin auth gate is enforced correctly on every admin route (401/403/200 across the full matrix), input validation and 404-before-any-Stripe-call behavior hold, and `/health` reports what it should. A second, Stripe-touching tier (one real round trip: create a customer, confirm a repeat call is idempotent) only runs when `/health` reports Stripe reachable, degrading to a clean SKIP rather than a false failure when run somewhere without Stripe network access (this sandbox). Wired as `npm run smoke-test -- --base-url <url>`; documented in `docs/RUNBOOK.md`'s Deploying section as a post-deploy step.

Manually proved both directions: ran it against a healthy local instance (43/44 passed, 1 correctly skipped for no Stripe network) and against a stopped one (43/44 failed with clear `fetch failed` errors, exit code 1) - confirming it would have caught the exact incident above automatically instead of by accident.

**Adversarial review (workflow, 2 parallel agents) found and fixed two real gaps before this shipped:**
- The generic auth-gate loop sent no request body to `/subscriptions/:id/plan`, `/cancel`, and `/dunning/:id/resolve`, so those three routes' write-key checks were silently satisfied by Zod's own 400 rather than ever reaching the local-lookup/404 path the test claimed to prove (still safe - no Stripe call was reached either way - but not proving what it claimed to prove). Fixed by sending a validly-shaped body referencing the fake id for each, so they now genuinely exercise the 404 path.
- The script's header comment and `docs/RUNBOOK.md` both claimed Tier 2 ran two round trips through Stripe (customer create, plus a real reconciliation run) but only the customer-create check was ever implemented - the reconciliation round trip didn't exist, wasn't run, and wasn't even reported as skipped when Stripe was reachable. Rather than add a second live-Stripe-touching check (which would only add another permanent side effect to worry about), corrected both the comment and the runbook to describe the one check that's actually there, and documented the trade-off it does have: the customer-create check has no teardown (there's no delete route anywhere in this API), so the first run against any given target permanently creates one real Stripe customer and one local row - safe to re-run (that's the idempotency being proved), but real, disclosed residue rather than a silent side effect.
...
## Infrastructure: the live-deployed-demo-seeding open item is resolved — `test-clock-demo.ts` run for real

**Date:** 2026-08-21

Run from a real Windows machine (Git Bash), not this sandbox - the same constraint every prior live verification in this log has hit. Reconfirmed directly this session rather than just assumed: this sandbox's outbound proxy explicitly policy-denies `api.stripe.com:443` (a logged `connect_rejected`, not a timeout), raw TCP Postgres connections aren't supported through it at all, and this session's Railway MCP tools returned `Not Authorized` - all three of the original Phase 9 blockers, independently reconfirmed rather than just re-asserted.

One setup wrinkle along the way, since Phase 10 added `ADMIN_API_KEY`/`ADMIN_READONLY_KEY` as required env vars after this machine's local `.env` was first set up: `npm run seed:catalog` failed at boot with `Invalid environment configuration` for both, even though neither `seed-catalog.ts` nor `test-clock-demo.ts` ever reads either one - `env.ts` validates the whole schema eagerly at import time regardless of which script triggered the import. Fixed by adding two local placeholder values (not the deployed service's real admin keys) to `.env`.

**`npm run seed:catalog`**, pointed at the connected Stripe test account (`acct_1TkxuuLVBwTnHcyi`), reported the same three products this account has carried since Phase 0's original catalog setup (`prod_UxrqtdT1NgAt7H`/`prod_Uxrq6NI6zt9vrU`/`prod_UxrqmunxHlblJK` for starter/pro/scale) - the script's own idempotent existing-product/existing-price search means this created nothing new, just wrote a fresh local `catalog.json`.

**`npx tsx scripts/test-clock-demo.ts`**, pointed at the real deployed Railway Postgres and the same Stripe account, ran the full arc clean on the first try - no fixes needed this time, unlike Phase 5's first live dunning verification, which surfaced three real bugs the first time anything ran against real Stripe infrastructure. In order: trial subscription created and synced; advanced past trial end, Stripe renewed and charged the good payment method (`in_1U71uFLVBwTnHcyiL7gkjed7` paid, subscription `active`); payment method swapped to Stripe's decline-on-charge test token; advanced past the next period end, the renewal failed (`in_1U71uZLVBwTnHcyi8IFw22dg`), a dunning cycle opened at stage 1 with its notice sent via the console adapter; two real `runDunningTick()` calls against the live Postgres (not backdated timestamps - genuinely later wall-clock-equivalent instants on the test clock's own timeline) escalated it to stage 2, then stage 3, one notice sent per stage, never twice; payment method swapped back and the failed invoice paid; the cycle resolved (`stage=0`, `noticesSent=3` preserved as history, matching D-013's one-notice-per-stage guarantee holding under a real tick schedule, not just the deterministic backdated-timestamp version `dunning.test.ts` already covered). The test clock and everything it created on Stripe were torn down (cascades customer/subscription/invoices); every local row the run created was cleaned up from the live Railway Postgres, confirmed by the script's own closing `[demo] local rows cleaned up` log line - nothing left behind on either side.

**This is the first time the full §5.12 arc - trial through renewal, failure, all three dunning stages, and recovery - has been proven end to end against real Stripe infrastructure and the actual deployed database in one continuous run**, not mocks, not backdated timestamps, and further than Phase 5's live verification went (that one stopped at stage 1, having proven the pipe itself worked; this run proves the full escalation ladder and recovery too). Closes the "Seeding the live deployed demo (Railway + Stripe)" open item carried since Phase 9. **No open items remain anywhere in this project.**
