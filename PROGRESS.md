...
**Open items now resolved:** item 1 above (live test-clock verification) is done. Remaining, unchanged:
...
3. Two test clocks from this verification session (`clock_1TxyWzLVBwTnHcyiEzY3mC8j`, `clock_1TxyYNLVBwTnHcyiyQkiIyyl`) and their throwaway customers/subscriptions need `teardownTestClock()` cleanup - not yet confirmed done as of this entry.
