...
This sandbox has no network path to `api.stripe.com` (unchanged since Phase 0), so these helpers are written and type-verified but not exercised against a real test clock from here. The dunning stage machine's escalation arc (stage 1 through terminal, then recovery) is instead verified deterministically in `test/integration/dunning.test.ts` by backdating `dunning_state.next_action_at` and calling `runDunningTick()` directly - proving the same tick logic a real test clock's time jump would exercise, without needing the network access to actually jump simulated time.
