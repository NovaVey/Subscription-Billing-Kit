...
**Date:** 2026-07-28 · **Phase:** 6 · **Status:** superseded by D-034
...
## D-033 — Admin access control is two shared keys, not user accounts
**Date:** 2026-07-29 · **Phase:** 10 · **Status:** settled
**Decision:** Every admin route (subscriptions, invoices, dunning, reconciliation, webhook-events admin endpoints) requires an `Authorization: Bearer <key>` header, checked against two env vars: `ADMIN_API_KEY` (full read+write) and `ADMIN_READONLY_KEY` (GET only - any mutating method with this key gets a 403). The public customer/checkout/portal routes and the webhook receiver are registered outside this scope in `app.ts` and stay ungated - Stripe's own signature already verifies the one, and the others are meant to be called by an external storefront, not an admin. Before this phase, every admin route had zero access control at all - not a missing tier, no gate whatsoever.
**Alternative rejected:** Full user accounts with sessions/login, or a single shared key with no read/write distinction, or SSO.
**Why it lost:** No phase in the original spec scoped authentication - this was flagged as an open item at the end of Phase 9 and built afterward on request. A full accounts system is real scope (a users table, session handling, a real login flow) for a kit whose spec never asked for it; a single undifferentiated key doesn't satisfy "read-only access," which was the specific gap raised. Two shared keys is the smallest change that actually delivers a genuine read/write distinction, matching this kit's existing pattern of doing the minimum the ask requires (see D-030's reasoning about not building a shared package for a single constant).
**Revisit if:** A client needs to know *which* individual made a change (the audit trail's `actor` column stays `'api'` regardless of which key was used - this doesn't distinguish callers, only permission level), or needs more than two tiers, or needs key rotation without redeploying - any of those is a real accounts system, not an extension of this gate.

## D-034 — Reconciliation windows are bounded by `invoices.created_at`, not `finalized_at`
**Date:** 2026-08-21 · **Phase:** post-launch bug hunt (medium-severity) · **Status:** settled
**Decision:** `invoices` gains a nullable `created_at` column, populated from `invoice.created` on every upsert in `handlers/invoice.ts`, and `runReconciliation()` now bounds its local query by that column instead of `finalized_at`.
**Alternative rejected:** Keep D-025's `finalized_at` windowing as-is.
**Why it lost:** `finalized_at` is null for any invoice still in `draft` status - a draft invoice was therefore invisible to reconciliation for however long it stayed a draft, the exact scenario D-025's own "Revisit if" clause named as the trigger to fix this with a schema change. A deep bug-hunt audit surfaced it directly (a draft invoice sitting in a reconciliation window silently never gets compared against Stripe's copy) rather than it being observed first via a real drift incident, but the mechanism is exactly what D-025 predicted. `invoice.created` (Stripe's own authoritative creation timestamp, present on every invoice regardless of status) is the correct "when was this issued" concept `stripe.invoices.list({ created })` already filters on for the Stripe-side half of the same comparison - the two sides of the comparison now use matching semantics.
**Known gap, accepted:** The new column is nullable and **not backfilled** for invoice rows that existed before this change shipped - every upsert in `handlers/invoice.ts` populates it going forward, so this self-heals for new activity, but a pre-existing invoice's `created_at` reads `null` (and is therefore still excluded from any window it should now match) until its next webhook event re-upserts the row. A companion `stripe_subscription_id` column was added in the same change for orphaned-invoice re-linking (see `docs/ARCHITECTURE.md`'s Phase 4 admin-actions sections) and carries the identical non-backfilled caveat.
**Revisit if:** The non-backfilled gap above ever causes a visible reconciliation mismatch on invoice data old enough to predate this migration - the fix would be a one-time backfill script reading `invoice.created` from Stripe for existing rows, not a further schema change.
