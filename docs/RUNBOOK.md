...
| `RECONCILE_TZ` | Optional (defaults `UTC`) | The timezone `computeYesterdayWindow()` bounds the nightly reconciliation window in. Validated as a real IANA time zone identifier at boot (`new Intl.DateTimeFormat(undefined, { timeZone: v })` must not throw) - an invalid value fails startup loudly instead of surfacing much later, mid-run, inside the nightly script. |
...
| `POST /checkout/sessions` or `POST /portal/sessions` returns 429 | The per-`customer_id`+IP rate limit (10 requests/minute, `lib/rateLimit.ts`) has been hit — deliberate abuse protection, since neither route authenticates the caller beyond a well-formed, existing `customer_id` | Expected under a retry storm or load test against the same customer id; not expected in normal single-user traffic. Raise the limit in `lib/rateLimit.ts` if a legitimate use case needs more than 10/minute per customer. |
